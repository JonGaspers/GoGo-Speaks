...
let DEEPSPEECH_MODEL = __dirname + '\\deepspeech-0.9.3-models\\deepspeech-0.9.3-models'; // path to deepspeech english model directory
...
function finishStream() {
	if (modelStream) {
		let start = new Date();
		let text = modelStream.finishStream(); // Recognized text for output
		if (text) {
			console.log('');
